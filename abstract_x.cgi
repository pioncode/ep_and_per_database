...
END
  ...
  #$thisip='146.87.255.19';   # !!!!! comment this out !!!!!!
  ...
  print p, "DIAGNOSTIC: $second, $minute, $hour, $dayOfMonth, $month, \$thisip=$thisip, \$ipcount=$ipcount";
  $uid=get_uid($matchcode);
  ...
  if (-e "$pdfpath{$journal}$subdir/$pdf"){  # test if PDF exists
    ...
    if ($uid){
      print p, "<a href=\"getpdf.cgi?id=$id\"><img src='Images/pdficon_small.gif' alt='PDF' border=0></a>&nbsp;<a href=\"getpdf.cgi?id=$id\" onClick=\"return popitup2('getpdf.cgi?id=$id')\"><b>Full-text PDF</b></a>&nbsp;<font size='-2'>size: $filesize Kb</font>";
      }
    elsif ($ipcount>0 || $pw==1){
