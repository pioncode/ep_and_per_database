...
$searchstring .= " and year>=$year" if $year =~ /^\d+$/;
...
unless ($query_orig =~ /\w+/ || $authorquery =~ /\w+/){
  ...
  if ($journal =~ /(P|V)/){
	...
	<p> 
      ...
      Words or phrases:<br>
      &nbsp;&nbsp;<input type=text name = query size=32 maxlength=25> <br>
      Author:<br>
      &nbsp;&nbsp;<input type=text name = authorquery size=32 maxlength=25> <br>   
      
  
      Select journal to search:
	<input type=radio name = journal $checked_P value ="P">&nbsp;Perception&nbsp;&nbsp;&nbsp;        
	<input type=radio name = journal $checked_V value = "V">&nbsp;ECVP abstracts&nbsp;&nbsp;&nbsp;
        <input type=radio name = journal value = "PV">&nbsp;both&nbsp;&nbsp;&nbsp;
      <p>
      Search all years<INPUT TYPE="Radio" NAME="volume1" CHECKED onClick="no()">
	  Search from a specific year to present<INPUT TYPE="Radio" NAME="volume1" UNCHECKED onclick="yes()">
...
END
	...
	<input type=radio name = journal value = "D">&nbsp;EPD&nbsp;&nbsp;&nbsp;<p>
	  ...
	  <input type=radio name = type checked value = "S">&nbsp;Subject&nbsp;&nbsp;&nbsp;
	  <input type=radio name = type value = "A">&nbsp;Author&nbsp;&nbsp;&nbsp;<p>
	...
	<label for="Year">
      ...
      Search for:
      <input type=text name = query size=32 maxlength=25>     
      <p>
      $pw_hidden
      <p><INPUT TYPE="SUBMIT" action="search.cgi">
      <INPUT TYPE="RESET" >
...
if($authorquery){ 
  ...
  if($authorquery=~/(\w)\w*\s+(\w+)/){
    ...
    $auquery_quoted= "last_name ilike ".$dbh->quote($authorquery.'%');
  } 
...
if ($count==$limit){
  ...
  print "<p><a href='search.cgi?journal=$journal&amp;year=$year&amp;type=$type&amp;issue=$issue$pw_string&amp;query=$query_orig&respage=$next_page'>More results</a>"}
