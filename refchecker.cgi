...
print p, "<body bgcolor=\"#FF0000\">";
  print p, "Checking references for: <font size=+2><b>$id</b></font><br>";
  if($id=~/(^p)/){
...
close (FILE);  
  open(FILE, $filename)|| die "couldn't connect";

while (<FILE>) {
  push(@fields,$_);  #adds each line to an array 
}
foreach (@fields) {
  chomp;
}
$onefile=join("", @fields);  #create a single variable containing the entire file
$onefile =~ s/fdb.*?<ref_head>(.*?)fdt.*/$1/g;
$onefile =~ s/\\\n\r/ /g;
$onefile =~ s/\\.\$//g;
$onefile =~ s/<>/<br>/g;
#
&cleanup; #run subroutine to clean up 3b2 stuff
@split = split(/<refs>/, $onefile);
##print "@split <p>";

foreach (@split) {
  if (/<i>Environment and Planning/ || /<i>Perception <\/i>/){
    print "<font color = #FF0000> <p>$_</font>";
    if (/^(.*?)(\d\d\d\d).{0,1}.*?&#8220;(.*?)&#8221;.*?<i>(.*?)<\/i>.*?(\d{1,2}).*?(\d{1,4})&nbsp;&#8211;&nbsp;(\d{1,4})/){
      $name = $1; #basic string with all names - needs to be more subtle?
      $year = $2;
      $title = $3;
      $journal_name = $4;
      $volume =$5;
      ...
      $journal =~ s/(.*?)nvironment and Planning (.{1})(.*)/$2/g;
#      print "|$1|$2|$3|$4|$5|$6|$7|";
      print "|$title|$volume|$start|<br>";

      $sth=$dbh->prepare("select count(*) from papers where volume=$volume and start_page=$start");
      $sth->execute() || die;
      ($count)=$sth->fetchrow();

      ################ LOOK FOR PAPER IN THE DATABASE ####################
      $sth=$dbh->prepare("select papers.paperid, journal, first_name, last_name, title, year, start_page, end_page, volume from papers, authors where journal=$journal and volume=$volume and start_page=$start");
      $sth->execute() || print "Failed with: prepare(\"select papers.paperid, first_name, last_name, title, year, start_page, end_page, volume from papers, authors where journal=$journal and volume=$volume and start_page=$start\");<br>";
      ($paperid, $fname, $lname, $title_db, $year_db, $start_page_db, $end_page_db, $volume_db)=$sth->fetchrow();

      if ($count < 1) {
	print "<br><font color=#0000FF>No matching record found in database</font>";
      } else {
	print "<font color=#0000FF>|$title_db|$volume_db|$start_page_db|</font><br>";
	if ($title_db =~ $title && $start_page_db =~ $start && $end_page_db =~ $end && $volume_db =~ $volume) { 
	  print "Reference matches database";
	} else {
	  print "<br>Should be:-<br>$lname $fname, \"$title_db\" <i>$journal</i> <b>$volume_db</b> $start_page_db - $end_page_db";
	}
#	if ($title_db =~ $title) { print "<br>Titles match perfectly";} else {print "<br><blink>Title should be |$title_db|</blink>";}
#	if ($start_page_db =~ $start && $end_page_db =~ $end) { print "<br>Page nos match perfectly";} else {print "<br><blink>Page nos should be |$start_page_db-$end_page_db|</blink>";}
#	if ($volume_db =~ $volume) { print "<br>Volume number matches perfectly";} else {print "<br><blink>Volume number should be |$volume|</blink>";}
      }
...
#        print "<br><font color=\"#0000FF\"> <!--$paperid--> $lname $fname, $year_db,  \"$title_db\" $journal <b>$volume_db</b> $start_page_db &#8211; $end_page_db <\/font>";
      ...
      print "Above reference does not match house style: check for an error (or maybe misidentified as a Pion journal?)."
    }
  }
}
...
sub cleanup   {   # cleans up 3b2 files
  $onefile =~ s/<\$>\\deg<\$>/&#176;/g; # is degree sign
  $onefile =~ s/<\$>//g;# deals with bogus
  $onefile =~ s/e\\x1B/&#233;/g;
  $onefile =~ s/è/&?acute;/g;
  $onefile =~ s/\\/ /g;
  $onefile =~ s/\\/ /g;
  ...
  $onefile =~ s/&#146;/ /g;
  $onefile =~ s/&#138;/&amp;/g;
  $onefile =~ s/--/&#8211;/g; #this is an em dash
  $onefile =~ s/ +/ /g;
  $onefile =~ s/"(\w)/&#8220;$1/g;
  $onefile =~ s/``/&#8220;$1/g;
  $onefile =~ s/(\w)"/$1&#8221;/g;
  ...
  $onefile =~ s/`(\w)/&#8216;$1/g;
  $onefile =~ s/(\w)'/$1&#8217;/g;
  $onefile =~ s/&emdash;/&#8212;/g; #this is an em dash
  #Italics and bold are difficult to encode owing to different combo's !
  $onefile =~ s/(<\?tf=&#8220;t002&#8221;>)(.{1,100})(<\?tf=&#8220;t005&#8221;>)(.{1,30})(<\?tf=&#8220;t001&#8221;>)/<i>$2<\/i><b>$4<\/b>/g; #is italic then bold combo
  $onefile =~ s/(<\?tf=&#8220;t002&#8221;>)(.{1,300})(<\?tf=&#8220;t001&#8221;>)/<i>$2<\/i>/g; #is italics-only combo
  $onefile =~ s/(<\?tf=&#8220;t005&#8221;>)(.{1,300})(<\?tf=&#8220;t001&#8221;>)/<b>$2<\/b>/g; #is bold-only combo
  $onefile =~ s/<\?twb>//g; #remove bogus rubbish
  $onefile =~ s/\&#38>;/&/g; #replace code for ampersand
  $onefile =~ s/&#142;\xF5d&#144;/<\/font>/g;# deals with dagger at end of title
#  $onefile =~ s/(<first>)(.*?)(<>)/<font size=3><b>$2<\/b><\/font><br>/g;# deals with headings
#  $onefile =~ s/(<second>)(.*?)(<>)/<b>$2<\/b><br>/g;# deals with headings
#  $onefile =~ s/(<third>)(.*?)(<>)/<i>$2<\/i><br>/g;# deals with headings
#  $onefile =~ s/<>/<p>/g;# deals with new paragraphs
  $onefile =~ s/(<first>)(.*?)(<>)/<h1>$2<\/h1>/g;# deals with headings
  $onefile =~ s/(<second>)(.*?)(<>)/<h2>$2<\/h2>/g;# deals with headings
  $onefile =~ s/(<third>)(.*?)(<>)/<h3>$2<\/h3>/g;# deals with headings
  $onefile =~ s/<>/<br>/g;# deals with new paragraphs
  $onefile =~ s/(&#142;)(.*?)(\(.*?\))/$2<font size=2><sup>$3<\/sup><\/font>/g;# deals with footnotes
  $onefile =~ s/&#TAB;//g;
  $onefile =~ s/&#1;//g;
}
