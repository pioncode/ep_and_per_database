...
if(($ENV{'HTTP_HOST'} ne 'www.envplan.com' && $ENV{'HTTP_HOST'} ne 'envplan.com' && $ENV{'HTTP_HOST'} ne 'envplan.co.uk' && $ENV{'HTTP_HOST'} ne 'proton5') && ($journal eq 'A' || $journal eq 'B' || $journal eq 'C' || $journal eq 'D')){
...
#  }
  ...
  # enable deep archive or 10-year backfile as applicable: 
   if ($year<$thisyear-10){
      if($journal eq 'P'){
        $access=$dbh->prepare("select count(*) from subscribers, access where subscribers.client=access.client and '$thisip'::CIDR <<= ip and (prod in ('XPE','XP') or (prod ~ '$journal.*$cy2' and $year > 1998) or prod ~ '^X.*$cy2'  or subscribers.client=9902 or (subscribers.client=10049 and $prodyear_xxxx>=1997) )");
       #print "select count(*) from subscribers, access where subscribers.client=access.client and '$thisip'::CIDR <<= ip and (prod in ('XPE','XP') or prod ~ '^X.*$current_sub' or subscribers.client=9902)";
        ...
        $access=$dbh->prepare("select count(*) from subscribers, access where subscribers.client=access.client and '$thisip'::CIDR <<= ip and (prod in ('XPE','XE') or (prod ~ '$journal.*$cy2' and $year > 1998) or prod ~ '^X.*$cy2'  or subscribers.client=9902 or (subscribers.client=10049 and $prodyear_xxxx>=1997) )"); 
      }
   } 
  else {
       $access=$dbh->prepare("select count(*) from subscribers, access where subscribers.client=access.client and '$thisip'::CIDR <<= ip and (prod ~ '$matchcode' or (prod ~ '$current_sub' and '$matchcode2' in ($ten_year_archive)) or subscribers.client=9902 or subscribers.client=10049)  and deny is not true");
       
       # was: $access=$dbh->prepare("select count(*) from subscribers, access where subscribers.client=access.client and '$thisip'::CIDR <<= ip and (prod ~ '$matchcode' or prod ~ '$grace_prod' or ((prod ~ '$current_sub' or prod ~ '$grace_prod') and '$matchcode2' in ($ten_year_archive)) or subscribers.client=9902)  and deny is not true");        
  } 
