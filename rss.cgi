...
$findlastvol=$dbh->prepare("select volume, max(issue) from issues where journal='$journal'  group by volume order by volume desc;") || die;
...
while (@result=$sth->fetchrow()){
  ...
  print <<END;  
<item rdf:about="http://$website/abstract.cgi?id=$paperid">
<title><![CDATA[$title. $authors]]></title>
<link>http://$website/abstract.cgi?id=$paperid</link>
